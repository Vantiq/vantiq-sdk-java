...
public class VantiqRequestTest extends VantiqTestBase {
    ...
    public void testUploadImageInvalidSessionHelper(String fileName, String contentType) throws Exception {
        ...
        File file = new File(this.getClass().getResource("/" + fileName).getFile());
        vantiq.upload(file, contentType, fileName, "/resources/images", handler);
        waitForCompletion();
    ...
    public void testUploadImageHelper(String fileName, String contentType) throws Exception {
        ...
        // Check response
        assertTrue("Successful response", handler.success);
        assertThat("Valid body - name", ((JsonObject) handler.getBody()).get("name").getAsString(), is(fileName));
        assertThat("Valid body - fileType", ((JsonObject) handler.getBody()).get("fileType").getAsString(), is(contentType));
    }
    ...
    public void testUploadVideoInvalidSession() throws Exception {
        ...
        File file = new File(this.getClass().getResource("/" + fileName).getFile());
        vantiq.upload(file, "video/mp4", fileName, "/resources/videos", handler);
        waitForCompletion();
    ...
    public void testUploadVideo() throws Exception {
        ...
        // Check response
        assertTrue("Successful response", handler.success);
        assertThat("Valid body - name", ((JsonObject) handler.getBody()).get("name").getAsString(), is("testVideo.mp4"));
        assertThat("Valid body - fileType", ((JsonObject) handler.getBody()).get("fileType").getAsString(), is("video/mp4"));
    }
}
